(() => {
  ...
  async function enhanceTrends() {
    ...
    panel.innerHTML = `
      ...
      <div class="grid grid-3" style="margin-top:12px">
        <div class="stat-card">
          ...
          <strong class="stat-value" style="font-size:16px">${escapeHtml(status?.source || "Google Trends Brasil")}</strong>
          <span class="stat-note">sinais públicos</span>
        </div>
        <div class="stat-card">
          <span class="stat-label">Frequência</span>
          <strong class="stat-value">${Number(status?.interval_minutes || 60)} min</strong>
          <span class="stat-note">sem ação manual</span>
        </div>
        <div class="stat-card">
          <span class="stat-label">Última busca</span>
          <strong class="stat-value" style="font-size:16px">${escapeHtml(formatDate(status?.last_run))}</strong>
          <span class="stat-note">${escapeHtml(summary(status?.last_result))}</span>
        </div>
      </div>
    ...
    document.getElementById("trendScanNow")?.addEventListener("click", async (event) => {
      const button = event.currentTarget;
      const message = document.getElementById("trendScanMessage");
      const original = button.textContent;
      button.disabled = true;
      button.textContent = "Buscando…";
      message.textContent = "Analisando tendências e criando oportunidades…";
      try {
        const result = await request("/trends/scan", { method: "POST", body: "{}" });
        message.textContent = `${Number(result.trends_saved || 0)} novas tendências e ${Number(result.opportunities_created || 0)} oportunidades criadas.`;
        setTimeout(() => location.reload(), 900);
      } catch (err) {
        message.textContent = err.message || "Não foi possível executar a busca agora.";
      } finally {
